...
papers <- papers_raw %>% 
  ...
  mutate(release = ifelse(status == "Published",
         ...
         # to published:
         status = fct_relevel(status, "In progress", "Under review", "Published"),
  ...
  mutate(status_order_i = 1:n(),
         display_label = ifelse(status_order_i == 1, release, "")) %>%
  ungroup()
...
```{r}
presentations %>%
  filter(presentation_type == "invited_seminar") %>%
  detailed_entries(what = title,
                   with = author_notes,
                   where = conference,
                   when = display_label,
...
```{r}
presentations %>%
  filter(presentation_type == "invited_industry") %>%
  detailed_entries(what = title,
                   with = author_notes,
                   where = conference,
                   when = display_label,
